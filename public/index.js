...
var material1 = new THREE.MeshBasicMaterial({
  map: loader.load(topLeftAtlasPath)
});

var material2 = new THREE.MeshBasicMaterial({
  map: loader.load(topRightAtlasPath)
});

var material3 = new THREE.MeshBasicMaterial({
  map: loader.load(bottomLeftAtlasPath)
});

var material4 = new THREE.MeshBasicMaterial({
  map: loader.load(bottomRightAtlasPath)
});
...
function buildGeometry(startX, startY, geometry) {
  for (var y = 0; y < atlas.rows; y++) {
    ...
    for (var x = 0; x < atlas.cols; x++) {
      ...
      var xOffset = x * image.width/atlas.width; //((x + 10 * y) % 10) * (image.width / atlas.width);
      ...
      var yOffset = y * image.height/atlas.height; //Math.floor((x + 10 * y) / 10) * (image.height / atlas.height);
...
buildGeometry(topLeftX, topLeftY, geometry1); 
buildGeometry(topLeftX + atlasOffset, topLeftY, geometry2);
buildGeometry(topLeftX, topLeftY - atlasOffset, geometry3);
buildGeometry(topLeftX + atlasOffset, topLeftY - atlasOffset, geometry4);
...
loadBase(buildingsJSONPath, imagesPath, 100);